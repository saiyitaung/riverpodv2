...
final popularActorsProvider = Provider((ref) => ActorRepo(dio: Dio(),api: Config.getAPIConfig()));
...
class ActorRepo{
  ...
  Future<List<Actor>> populars() async {
    ...
    List<Actor> actors = [];
    actors = List<dynamic>.from(resp.data['results']).map((e) => Actor.fromJson(e)).toList();
  ...
  Future<List<MovieCast>> moviesCastByPerson(int id) async{
    final resp = await dio.get("$baseActorURL/$id/combined_credits",queryParameters: {
      "api_key":api.key,
      "language":"en-US",
    });
    List<MovieCast> mc = [];
     
    mc = List<Map<String,dynamic>>.from(resp.data['cast']).map((e) => MovieCast.fromJson(e)).where((element) => element.backdropPath != null).toList();
